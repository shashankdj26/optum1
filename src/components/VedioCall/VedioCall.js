...
class VedioCall extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            pinActive: false,
            pinSid: null,
            // roomJoinedOnce: false,
            callState: CallStates.init,
        };
    ...
    componentDidMount() {
        ...
        axios.post('https://twilioserver-dot-virtualeventdemo.el.r.appspot.com/api/token',    //'https://videocall.photoboothassetmanagement.co.in/api/token',   //'https://iijs-token-server.djvirtualevents.com/api/token',
            { name: userName })
            .then(results => {
            ...
            .catch(error => {
                ...
                this.setState({ callState: CallStates.roomJoinFailed });
    ...
    joinRoom() {
     ...
     Show an error message on room name text field if user tries         joining a room without providing a room name. This is enabled by setting `roomIdErr` to true
       */
        ...
        this.setState({ callState: CallStates.joiningRoom });
    ...
    roomJoined(room) {
        ...
        this.setState({ activeRoom: room, localMediaAvailable: true, hasJoinedRoom: true, callState: CallStates.joinedRoom });
        ...
        room.on('disconnected', () => {
            ...
            this.setState({ screenShared: false, remoteScreens: [] });
    ...
    participantConnected(participant) {
        ...
        var gridSize = Math.floor(Math.sqrt(participants.length + 1));
        ...
        if (!this.props.maximized) {
            var padding = 0.5 / gridSize;
            var fontSize = 2.0 / gridSize;
            if (gridSize < 2)
                fontSize = 1.0;
            console.log(padding);
            console.log(fontSize);
            span.style.padding = padding + 'rem';
            span.style.fontSize = fontSize + 'rem';

            var allSpans = document.getElementsByClassName("callingBox__nameTag");
            for (var i = 0; i < allSpans.length; i++) {
                allSpans[i].style.padding = padding + 'rem';
                allSpans[i].style.fontSize = fontSize + 'rem';
            }
        }
    ...
    leaveRoom() {
        ...
        if (this.state.activeRoom) {
            ...
            this.setState({ hasJoinedRoom: false, localMediaAvailable: false, callState: CallStates.disconnected });
        }
    ...
    rejoinRoom = () => {
        if (this.state.callState === CallStates.disconnected) {
            this.joinRoom();
        }
        else if (this.state.callState === CallStates.roomJoinFailed) {
            this.setState({ callState: CallStates.init });
    ...
    generateGrid = () => {
        ...
        if (length <= 1) {
            ...
            view = "";
            var sqRoot = Math.floor(Math.sqrt(length) + 1);
            for (var i = 0; i < sqRoot; i++) {
                view += "auto ";
            }
            view += "auto";
            console.log(length + " " + sqRoot);
        }
        return view;
    }
                ...
                <div className={`callingBox__body  h-100 ${this.state.splitScreen ? ' callingBox__body-right__active ' : ''}`}>
                    ...
                    <div className={`callingBox__left h-100 ${this.state.screenShared || remoteShared || this.state.pinActive ? "pin-video-active" : ""}`}>
                        ...
                        <div className={`shared-screenBox has-video ${this.state.screenShared || remoteShared ? "active" : "d-none"}`} id="screen-share" >
                            <button className={`full-screen-btn ${this.state.screenShared || remoteShared ? "active" : "d-none"}`} onClick={this.openFullscreen}>
                                <i className="icon-full-screen"></i>
                            </button>
                        </div>

                        {/* <div className="active" id="screen-share" style={{ position: "absolute", bottom: "0.25rem", right: "0.5rem", zIndex: "10" }}>
                            <div className="full-screen-btn" onClick={this.props.toggleMaximized} style={{ color: "white", fontSize: "1.5rem" }}>
                                <i className="icon-full-screen"></i>
                            </div>
                        </div> */}
                        ...
                        <CallMenu
                            ...
                            isAudioEnabled={this.state.audioEnabled}
                            isVideoEnabled={this.state.videoEnabled}
                            isLocalScreenSharing={this.state.screenTrack != null}
                            isRemoteScreenSharing={this.state.remoteScreens.length > 0}
                    ...
                    {/* <div className={`callingBox__right h-100 ${!this.state.splitScreen ? ' d-none ' : ''}`}>
                        ...
                        <VideoCallChat showNotification={value => this.showNotification(value)}
                            ...
                            className={`${this.state.activeSideMenu != 'Chat' ? ' d-none ' : ' '}`}></VideoCallChat>
