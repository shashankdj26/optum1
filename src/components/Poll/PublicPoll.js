...
class PublicPoll extends Component {
    ...
    suscribeToPollAdmin = () => {
        ...
        this.removeListener = firestore.collection(this.props.PollAdmin_Col).doc(this.props.PollAdmin_Doc).onSnapshot(
            doc => {
        ...
        if (this.pollDocData.length > 0) {
            ...
            this.pollDocData.forEach((docData) => {
                if (!docData.data().disabled) {
                    pollValues.push(
                        <div className={`poll-section__block poll-identifier`} id={`${docData.id}`} key={`${docData.id}`}>
            ...
            pollValues.push(
                <div className="poll-section__block" key={'submitConainer'}>
                    <div className="pd-t10 pd-b10">
                        <button className="poll-btn btn-primary" onClick={this.onSubmit}>Submit</button>
                    </div>
                </div>
            );
            return pollValues;
            ...
            pollAnswers.push(
                <div className="poll-custom-progress-box" key={options.indexOf(option)}>
                    ...
                    <div className="poll-custom-progress-bar">
                        <div className="poll-custom-progress-bar__front" style={{ width: `${percentage}%` }}></div>
                        <span className="poll-custom-progress-bar__percent"> {`${percentage}%`}</span>
                    </div>
                </div>
            )
        });
...

                        {
                            (this.state.pollCurrentState === pollStates.waitForResults) ?
                                (<div className="d-flex h-100 align-items-center justify-content-center">
                                    <p className="light-color">Waiting for results</p>
                                </div>)
                                : (null)
                        }
                        {
