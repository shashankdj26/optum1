const pollAdminCol = 'pollAdmin';
const pollAdminDoc = 'adminAccess';
const pollUserDoc = 'poll';
...
function addPollFirebase(question, option) {
    ...
    db.collection(pollUserDoc).add({
        ...
        .then(function (docRef) {
            ...
            addPollInPage(question, option, docRef.id, false)
...
function readAddedPoll() {
    db.collection(pollUserDoc).get().then(snapshot => {
        ...
        docs.forEach(doc => {
            ...
            addPollInPage(pollData.question, pollData.option, doc.id, pollData.disabled);
        });
        // attachListener()
    });
}
...
function addPollInPage(question, options, pollId, disabled) {
    ...
    var dcontinaer = document.createElement('div');
    dcontinaer.classList.add('d-flex');
    footer.appendChild(dcontinaer);
    ...
    dcontinaer.appendChild(deleteButton);

    var toggleBtn = document.createElement("button");
    toggleBtn.id = pollId + "_toggleButton";
    toggleBtn.className = "toggleButton btn " + (disabled ? "btn-secondary" : "btn-primary");
    toggleBtn.innerHTML = disabled ? "Make Active" : "Make Deactive";
    ...
    toggleBtn.onclick = async function () {
        ...
        if (!pressed) {
            pressed = true
            await toggleQuesstion(pollId)
            updateSavePoll();
            pressed = false
        }
    };
    dcontinaer.appendChild(toggleBtn);
...
function changePublishButton(value) {
    if (value) {
        ...
        document.querySelectorAll(".toggleButton").forEach(element => {
...
function updateSavePoll() {
    db.collection(pollUserDoc).get().then(snapshot => {
        ...
        docs.forEach(doc => {
            ...
            var toggleBtnId = docId + "_toggleButton";
            var toggleBtn = document.getElementById(toggleBtnId);
            toggleBtn.className = "btn " + (docData.disabled ? "btn-secondary" : "btn-primary");
            toggleBtn.innerHTML = docData.disabled ? "Make Active" : "Make Deactive";
...
function attachListener() {
    db.collection(pollUserDoc).onSnapshot(snapshot => {
        var docs = snapshot.docs; //to get document
        docs.forEach(doc => {
            const docData = doc.data();
            var docId = doc.id;
...
//----------------------
function toggleQuesstion(pollId) {
    return new Promise(async (res, rej) => {
        try {
            const docRef = db.collection(pollUserDoc).doc(pollId)
